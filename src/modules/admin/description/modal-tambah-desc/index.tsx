...
import { usePostCreateDescription } from "@/src/hooks/description/hook";
import { modalCreateDescriptionOpenState } from "@/src/recoil/atoms/descriptions";
...
export const CreateDescriptionModal: FC = () => {
  const [isModalOpen, setIsModalOpen] = useRecoilState(
    modalCreateDescriptionOpenState
  );
  const queryClient = useQueryClient();
  const { mutate } = usePostCreateDescription();
    ...
    try {
      mutate(formData as unknown as TCreateDescriptions, {
        onSuccess: () => {
          ...
          toast.success("Create Description Success!", {
            autoClose: 2000,
          });
        },
      });
